...
rasterToPoints <- function(x, fun=NULL, spatial=FALSE, ...) {
	...
	if (! inherits(x, 'RasterStack' )) {
		if ( ! fromDisk(x) & ! inMemory(x) ) {
			if (spatial) {
				return(sp::SpatialPoints(coords=xyFromCell(x, 1:ncell(x)),  proj4string=projection(x, asText=FALSE)) )
			} else {
	...
	if (spatial) {
		if (nrow(xyv) == 0) {
			...
			colnames(v) <- laynam
			s <- sp::SpatialPointsDataFrame(coords=xyv[,1:2,drop=FALSE], data=v,  proj4string=projection(x, asText=FALSE) )
			return(s[0,])
		} else {
			v <- data.frame(xyv[ ,-c(1:2), drop=FALSE])
			colnames(v) <- laynam
			return( sp::SpatialPointsDataFrame(coords=xyv[,1:2,drop=FALSE], data=v,  proj4string=projection(x, asText=FALSE) ) )
		}
